...
який слід викликати з методу` __init__` для перевірки
електронної пошти параметра, переданого конструктору.
Логіка методу `перевірка` може полягати в тому, щоб перевірити,
 чи переданий параметр електронної пошти є дійсним рядком
  електронної пошти.
'''
